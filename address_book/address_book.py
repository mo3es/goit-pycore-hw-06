...
class Record:
        ...
        for p in self.phones:
            if p.value == phone:
                if new_phone not in [p.value for p in self.phones]:
                    p.value = new_phone
                    print(f'Inputed phone number {phone} successfully edited to {new_phone}')
                else:
                    print(f'Phone number {new_phone} already exists, phone number {phone} was not edited')
                return
    ...
    def find_phone(self, phone):
        if phone in [p.value for p in self.phones]:
            ...
            print(f'In {self.name.value} contact info, phone number {phone} does not exist')
